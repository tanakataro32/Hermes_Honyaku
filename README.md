...
- **縦向き**: 上に貼り付いた欄 (タイトル・ランプ・[…] / コンテキストメーター / GPU / 原文の最新 3 行) と、その下の残りの高さで流れる和訳。
  - **GPU** の行を押すとツリーを閉じられ、和訳の見える範囲が広がります。閉じても GPU ごとに「温度・VRAM・電力」の 1 行の要約は残ります (カードごとの行を押して、そのカードだけ閉じることもできます)。開閉は覚えておきます。
...
- **和訳の流れ**: ターンの枠はなくし、ターンの変わり目に細い見出し (番号・時刻・発信元・状態) を挟みます。ツール行 (🔧) と最終回答も流れに入ります。
  翻訳待ちの文は出さず「翻訳中… (待ち n)」の 1 行にまとめ、訳し終わった所までを出します。新しい訳文が来ると滑らかに流れて追いかけます。

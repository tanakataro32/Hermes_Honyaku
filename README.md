...
画面は左右 2 列です (スマホ表示は「4a」)。

```
┌ タイトルバー (バージョン・表示設定のチェック・発信元の絞り込み・画面を消去・📚 本棚) ┐
├ 左 30% (上下に固定) ─────┬ 右 70% (ページごと流れる) ───────────────────────────┤
│ 接続・翻訳サーバー        │ [和訳] #12 Hermes            翻訳中… (待ち 3)      │ ← 固定
│ コンテキスト              │ #11 10:20 Hermes … 完了                            │
│ システム (GPU・CPU・RAM・ │ 訳文 訳文 訳文 …                                  │
│   ディスク / 黄・赤ボタン) │ 🔧 ツール行 / 回答 (ティール色の枠)                │
│ [原文] #12 · 推測中 ■停止 │ #12 10:21 Hermes … 思考中… ■停止                   │
│ 原文の最新の行            │ 訳文 …                                            │
└──────────────────────────┴───────────────────────────────────────────────────┘
```

- **ターンの見出し** (和訳の流れの区切り): 「通し番号 / 時刻 / 発信元 / モデル名 / **コンテキストメータ** / 直前のメッセージの要約 (user: … や tool(名前): …) / 状態」。
  Hermes はツールを呼ぶたびに LLM に問い合わせるので、1 つの指示で複数ターンになります。
  **見出しを押すと、そのターンの原文 (Thinking) 全体が訳文の上に開きます** (もう一度押すと閉じる。開いたときは読めるように自動スクロールを止めます)。
- **左の列** (スクロールしても固定): 接続状態・翻訳サーバー状態・コンテキストメータ・システム (GPU・CPU・RAM・ディスク) と、その下の**原文の欄**。
  原文の欄は、推測中でいちばん新しいターンの原文の最新の行を、空いた高さいっぱいに小さめの字で出します (背景タスクを隠す・発信元の絞り込みを反映。推測中が無ければ最後のターンを薄く)。
...
- **GPU メータ** (左の列のシステム): `nvidia-smi` から 5 秒ごとに読み取った GPU の**温度**・**VRAM 使用量** (メモリのアイコン)・**消費電力**を、
  カード 1 枚ずつプログレスバー 3 本で表示します (表示名は型名に省略)。GPU やカードの行を押すと閉じ、閉じたカードは 1 行の要約 (温度・VRAM・電力) になります (開閉は覚えておきます)。
...
- **Hermes を止めるボタン** (左の列の GPU 行の右端に 2 つ。マウスを乗せると説明が出ます): サーバーで hstop を実行し、
  出力 (止めた作業・本体の再起動・GPU の状態) をダイアログで表示します。AI モデル (llama-server) と中継サーバー自身には触りません。
  - **黄色 ↻ = `hstop --pid …`**: 動いている Hermes の裏の作業 (cron の定期実行、本棚などのアプリが頼んだ AI の作業) の一覧
    (`hstop --list`、2 秒ごとに更新) が画面の右上に出るので、止めるものにチェックを付けて「選んだ作業を止める」。
    1 行に 頼んだもの (本棚なら hondana、cron など)・動いている時間・依頼の冒頭・その作業のターン番号 (押すとそのターンへ移動) を表示します。
    開いている間は、作業ごとの色で「その作業が推測中のターンの見出し」を塗るので、どれを止めるか画面で見比べられます
    (行にマウスを乗せると濃くなる)。開いたときにあった作業は最初からチェック済み、開いている間に増えた作業はチェックなし (「新」)。
    デスクトップアプリの会話は続きます。一覧の下には参考として、ほかに推測中のもの (デスクトップアプリの会話、Open WebUI など) と
    その止め方を出します (ここでは止められません)。
  - **赤 ↻ = `hstop --all`**: 裏の作業に加えて Hermes の本体 (ダッシュボード) を起動し直すので、デスクトップアプリの会話も止まります (自動で再開しません)。
    終わったらデスクトップアプリからつなぎ直します。迷ったらこちら。
  ...
  黄色のボタンには `--list` / `--pid` が使える hstop が要ります (古いときはダイアログに出るので、llmsv で `deploy --self-update`)。
  どのターンがどの作業かは、中継サーバーが接続元のプロセスを /proc から調べて結び付けます (Hermes と同じユーザー・同じマシンで動かしているときだけ)。
- **推測中のターンの停止ボタン** (原文の欄の右上と、推測中のターンの見出しの右端。どちらも生成される文字に押されて動くことはありません):
  その推測 (1 回の要求) だけを止めます。
  誤操作よけに 2 回押します (1 回目で赤く点滅し、3 秒以内にもう一度押すと止まる)。中継サーバーが llama-server と Hermes との接続を切るので
  GPU はすぐ止まりますが、Hermes からは通信エラーに見えるため、同じ要求をやり直したり次の手順に進んだりして、作業自体は続くことがあります。
  作業ごと止めるときは黄色のボタン、会話ごと止めるときは赤のボタン。止めたターンは「停止ボタンで中断」と表示されます。
  やり直しよけ: 止めた要求とまったく同じ要求 (モデルと会話が同じ) が同じ接続元から 2 分以内に来たら、推測させずに HTTP 400
  (「停止ボタンで止めた要求です」) を返し、画面に「やり直しを断りました」と出します (断るたびに 2 分延長)。
  400 はふつうやり直しの対象にならないので、Hermes はその回答を諦めてエラーで終わるはずです。自分で送り直したメッセージは内容が違うので断りません。
...
- **和訳の流れ** (右の列): 1 つの枠の中に、文の区切りごとの訳文を段落のように並べます (1 文ごとの枠はありません)。
  翻訳待ちの文とその後ろは出さず、訳し終わった所までが流れます。待ちの数は枠の一番上のタイトル行 (`翻訳中… (待ち 3)` / `追いついています`) に出ます。
  翻訳が追いつかないときも原文の欄は止まらず、和訳が順に追いつきます。
- そのターンで Hermes が呼んだツール (実行したコマンドなど) は、訳文の後ろに 🔧 付きで 1 行ずつ出ます。
- Hermes の最終回答はそのターンの一番下に「回答」としてティール色の枠で Markdown で描画されます (表・箇条書き・コードブロックなど)。回答は元から日本語なので翻訳は通しません。不要なら「回答も表示」のチェックを外します。
- モデルが日本語で考えている文は翻訳せずそのまま出します。翻訳モデルが英語のまま返してきた文は、左に黄色の線を付けて原文を表示します。
- 思考の中のシェルコマンドやプログラムコード (``` で囲まれたブロック、`cd ... && ...` のようなコマンド行、Python / JS / VB / SQL などのコード行) は翻訳せず、
  等幅フォント・左に灰色の線でそのまま出します (小型の翻訳モデルに渡すと壊れるため)。直前の「Command:」のような短い見出し行もコードと一緒にまとめます。
  コード行は長くても分割しません。判定は行頭のコマンド名・シェル記号・言語の構文で自動で行い、足りないコマンド名は `config.ini` の `[segment] code_words` に追加できます。
- 翻訳待ちのまま次のターンが始まった文は、翻訳を打ち切って原文を黄色の線付きで表示します (新しいターンの翻訳を優先するため。タイトル生成などの背景タスクの開始では打ち切りません)。
- 自動スクロールは、訳し終わった最新の所を滑らかに流れて追いかけます。原文が伸びても追従しないので、翻訳が追いついている位置で止まります。
  上へ戻る操作 (ホイール上・↑ / PageUp / Home・スクロールバーを掴む・指で下に引く) をすると自動スクロールは止まり、最新の位置かページの一番下まで戻すと再開します
  (左の列の中でホイールを回しても止まりません)。
...
## 4a. スマホで見る (iPhone 向け表示)

iPhone の Safari で同じ URL (`http://192.168.1.50:8765/`、Tailscale なら `http://<llmsv の Tailscale IP>:8765/`) を開くと、自動でスマホ向けの表示になります。
`/m` を付けて開くと (`http://192.168.1.50:8765/m`) どの端末でも必ずスマホ表示になります。眺めているだけで GPU のメーターが動き、訳文が流れていく作りです。

- **縦向き**: 上に貼り付いた欄 (タイトル・ランプ・[…] / コンテキストメーター / GPU / 原文の最新 3 行) と、その下の残りの高さで流れる和訳。
  - **GPU** のメーターは 温度・VRAM (メモリのアイコン)・電力 の順です。GPU の行を押すとツリーを閉じられ、和訳の見える範囲が広がります。閉じても GPU ごとに「温度・VRAM・電力」の 1 行の要約は残ります (カードごとの行を押して、そのカードだけ閉じることもできます)。開閉は覚えておきます。
  - **原文の欄**は、いま推測中のターンの最新の行を小さい字で出します (背景タスクを隠す・発信元の絞り込みを反映)。欄を押すと 3 行 ⇔ 10 行を切り替えます。
    推測中のあいだは欄の右上に**停止ボタン** (そのターンだけ止める。PC と同じく 2 回押す) が出ます。
- **横向き**: 左 30% に同じ欄 (原文は空いた高さいっぱいに出す)、右 70% で和訳が流れます。
- **和訳の流れ**: 原文の欄と同じ枠 1 つにまとめ、1 文ごとの枠は付けずに段落のように並べます。ターンの変わり目には細い見出し (番号・時刻・発信元・状態) を挟みます。
  ツール行 (🔧) と最終回答 (ティール色の枠) も流れに入ります。英語のまま返ってきた文は左に黄色の線、コードは等幅の字で左に灰色の線、エラーは赤い字です。
  翻訳待ちの文は出さず、訳し終わった所までを出します。新しい訳文が来ると滑らかに流れて追いかけます。
  枠の一番上の**タイトル行** (`和訳 #3 Hermes … 翻訳中… (待ち 3)` / `追いついています`) はいつも見えていて、翻訳待ちの数を出します。
  指で下に引いて上 (前の訳文) に戻すと追いかけるのを止め、右下の「最新へ」か、指を離して最新の位置まで戻ると再開します
  (止まるのは指で下に引いたときだけです。メニューの中や横向きの左の欄を指で動かしても止まりません)。
  勝手に止まるときは、[…] の「自動スクロール」の下に出る「最後に止まった理由」を見てください。
- **上のバー**: 左のランプは「中継」(この画面と中継サーバーの接続) と「翻訳」(翻訳サーバー。黄色は待ちあり、`待3` = 3 文待ち)。
- **[…] のメニュー**: 接続の詳細、CPU・RAM・ディスク、表示の設定 (自動スクロールなどのチェックと発信元の絞り込み、画面を消去)、画面を消さない、
  本棚を開く (新しいタブ。URL は config の `[ui] hondana_url`)、PC 表示に切り替える、バージョン。
- **黄・赤のボタン**は PC と同じ動きです。黄色の一覧は画面の下から出ます (開いている間は、作業ごとの色でターンの見出しを塗ります)。
- **PC 表示に戻す / スマホ表示にする**: […] の「PC 表示に切り替える」で PC 表示になり、その端末では次からも PC 表示で開きます。
  スマホ表示に戻すときは、PC 表示のタイトルバーの右端の「📱 スマホ表示」(指で操作する端末だけに出ます) を押すか、`/m` を開きます。
- **画面を消さない**は、ブラウザの「画面を点けたままにする」機能 (Screen Wake Lock) を使います。iPhone では **https で開いたときだけ**使えるので、
  `http://` で開いている間はチェックできません。その場合は iPhone の「設定 > 画面表示と明るさ > 自動ロック」を「なし」にして代わりにしてください。

### ホーム画面に追加する (全画面のアプリのように開く)

Safari のアドレスバーやタブのバーが消えて、画面を広く使えます (特に横向きで効きます)。

1. iPhone の Safari で `http://192.168.1.50:8765/` を開きます (スマホ表示になっていることを確認)。
2. 下のバーの**共有ボタン** (□ に ↑ のアイコン) を押します。
3. 一覧を下に送って**「ホーム画面に追加」**を押します (見つからないときは一覧の一番下の「アクションを編集…」から追加できます)。
4. 名前が「Honyaku」になっているのを確認し (変えても構いません)、iOS 16.4 以降なら「Web アプリとして開く」がオンになっていることを確かめて、右上の**「追加」**を押します。
5. ホーム画面にできた「Honyaku」のアイコンから開きます。以後は Safari のバーなしで開きます。

- ホーム画面から開いた画面は Safari とは別扱いなので、表示の設定 (チェックなど) は Safari で開いたときと別に覚えます。
- Tailscale 経由でも見たい場合は、Tailscale の IP の URL でもう 1 つ追加しておくと便利です (名前を「Honyaku (外)」などに変える)。
- llmsv の IP が変わったときは、アイコンを長押しして削除し、追加し直します。
...
| ui | hondana_url | スマホ表示の […] のメニューに出す本棚へのリンク (既定 `http://llmsv/hondana`。空欄でリンクを出さない) |
...
| hstop | path | GPU 行のボタン (黄 = hstop --list / --pid / 赤 = hstop --all) で実行する hstop の場所。空欄なら PATH → `~/.local/bin/hstop` の順に探す |
| hstop | timeout | hstop がこの秒数 (既定 120) で終わらなければ打ち切ってエラー表示 |

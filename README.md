...
- **Hermes を止めるボタン** (システムパネルの GPU 行の右端に 2 つ。マウスを乗せると説明が出ます): 押すと確認のあと、サーバーで hstop を実行し、
  出力 (止めた作業・本体の再起動・GPU の状態) をダイアログで表示します。AI モデル (llama-server) と中継サーバー自身には触りません。
  - **黄色 ↻ = `hstop`**: Hermes の裏の作業 (cron の定期実行、本棚などのアプリが頼んだ AI の作業) だけを止めます。デスクトップアプリの会話は続きます。
...
| hstop | path | GPU 行のボタン (黄 = hstop / 赤 = hstop --all) で実行する hstop の場所。空欄なら PATH → `~/.local/bin/hstop` の順に探す |

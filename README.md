...
- **GPU メータ** (システム情報パネル): `nvidia-smi` から 5 秒ごとに読み取った GPU の**温度**・**消費電力**・**VRAM 使用量**を、カード 1 枚ずつ表示します
  (例 `V100 55W` + 温度バー 78℃ + VRAMバー 30.3/32G。表示名は型名に省略)。温度・VRAM はコンテキストメータと同じプログレスバーで表示します。
  温度 90℃ 満杯で 75℃ 以上黄色・85℃ 以上赤、VRAM は使用率で埋まり 95% 以上で黄色になります。
...
| gpu | interval | ヘッダーの GPU メータ (温度・VRAM) の読み取り間隔 (既定 5)。nvidia-smi がない環境では自動で表示なし |

...
- **Hermes を止めるボタン** (システムパネルの GPU 行の右端に 2 つ。マウスを乗せると説明が出ます): サーバーで hstop を実行し、
  ...
  - **黄色 ↻ = `hstop --pid …`**: 動いている Hermes の裏の作業 (cron の定期実行、本棚などのアプリが頼んだ AI の作業) の一覧
    ...
    開いている間は、作業ごとの色で「その作業が推測中のターンの原文」を塗るので、どれを止めるか画面で見比べられます
...
- **推測中のターンの停止ボタン** (「Thinking (原文)」の見出しの右端。見出しの行はヘッダーの下に貼り付くので、原文が長くても
  そのターンが画面にある間は見えていて、生成される文字に押されて動くこともありません): その推測 (1 回の要求) だけを止めます。
...
- **黄・赤のボタン**は PC と同じ動きです。黄色の一覧は画面の下から出ます (スマホでは、原文を作業の色で塗って見比べる機能はありません)。
